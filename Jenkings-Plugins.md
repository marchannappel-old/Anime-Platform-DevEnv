# Jenkins Plugins

- [Dependency Analyzer](https://plugins.jenkins.io/dependencyanalyzer/)
- [Maven Repository Server](https://plugins.jenkins.io/repository/)
- [JUnit Realtime Test Reporter](https://plugins.jenkins.io/junit-realtime-test-reporter/)
- [Config Driven Pipeline](https://plugins.jenkins.io/config-driven-pipeline/)
- [Mailer](https://plugins.jenkins.io/mailer/)
- [Gitlab](https://plugins.jenkins.io/gitlab-plugin/)
- [Gitlab API](https://plugins.jenkins.io/gitlab-api/)
- [Gitlab Branch Source](https://plugins.jenkins.io/gitlab-branch-source/)

## External Tool Integration

- [Artifactory](https://plugins.jenkins.io/artifactory/) 
- [SonarQube](https://plugins.jenkins.io/sonar/)
- [Nexus](https://plugins.jenkins.io/nexus-jenkins-plugin/)
- [Gatling Load Testing](https://plugins.jenkins.io/gatling-check/)
- [Snyk Security](https://plugins.jenkins.io/snyk-security-scanner/)
